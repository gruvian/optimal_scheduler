...
import json, subprocess, os, greedy_algorithm
...
class Gui(QtWidgets.QWidget):
    ...
    def init_gui(self):
        ...
        if not os.path.exists("timetable_data.json"):
    ...
    def save_schedule(self):
        ...
        filename, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save Timetable", "", "JSON Files (*.json);;All Files (*)", options=options)
        if filename:
            with open("timetable_data.json", "r") as f:
                timetable_json = json.load(f)
        ...
        for course in study_courses:
            ...
            while remaining_time < allocated_time:
                if slot > 11:
                    ...
                    k += 1
                    slot = end_of_work[k] + 2
                self.add_label(course["course_name"], slot, i, "background-color:" + colors[l] + ";")
                remaining_time += 1
                slot += 1
            ...
            k += 1
            l += 1
        self.add_button("View graph", 12, 7, self.show_time_distribution)
    ...
    def create_new_schedule_timetable(self):
        ...
        for row in range(0, 5):
            ...
            performance_json.append({"course_name": self.courses[row].text(), "ECTS": int(ects), "grade": float(self.grades[row].text())})

        with open("previous_performance.json", "w") as f:
            json.dump(performance_json, f)

        self.create_course_optimized_schedule()

        self.display_schedule("timetable_data.json")
    ...
    def view_old_schedule(self):
        ...
        self.path_to_json = self.add_line_edit("timetable_data.json", 3, 0)
    ...
    def create_course_optimized_schedule(self):
    ...
    def error_message(self):
        ...
        self.dark_mode_flag = self.detect_dark_mode_gnome()
        if self.dark_mode_flag:
            self.setStyleSheet(open('night_theme.css').read())
        else:
            self.setStyleSheet(open('day_theme.css').read())
